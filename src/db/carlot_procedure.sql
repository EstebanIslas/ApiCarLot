DROP PROCEDURE IF EXISTS `registroParks`;
delimiter ;;
CREATE PROCEDURE `registroParks`(IN `_id` INT,
...
DROP PROCEDURE IF EXISTS `registroReservas`;
delimiter ;;
CREATE PROCEDURE `registroReservas`(IN `_id` INT,
	IN `_id_park` INT,
	IN `_id_user` INT
,
...
delimiter ;;
CREATE PROCEDURE `registroUsuarios`(IN _id INT,
  IN _nombre VARCHAR(50),
  IN _apellido VARCHAR(50),
  IN _telefono VARCHAR(15),
  IN _correo VARCHAR(60),
